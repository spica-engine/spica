...
function build(meta: BuildMeta) {
  const entryPoint = path.join(meta.cwd, meta.entrypoints.build);
  const outDirAbsolutePath = path.join(meta.cwd, meta.outDir);
  const outDirRelative = meta.outDir;
  const currentDirectory = meta.cwd;
  ...
  program.emit();
  postBuild(currentDirectory, outDirAbsolutePath, diagnostics);
}
...
function postBuild(baseUrl: string, outDir: string, diagnostics: ts.Diagnostic[]) {
