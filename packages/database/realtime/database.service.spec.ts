    ...
    describe("with filter", () => {
      ...
      it("should return added documents", async done => {
        await database
          ...
          .insertMany([
            {stars: 3},
            {stars: 4, anotherfilter: true},
            {stars: 5, anotherfilter: true}
          ]);
        realtime
        ...
        await database
          .collection("test6")
          .insertMany([{stars: 6}, {stars: 7, anotherfilter: true}]);
      });
  ...
  describe("changestream management", () => {
    ...
    describe("removeEmitter", () => {
      it("should only descrease listener count", () => {
        ...
        const emitter = emitters.get(emitterName);
      ...
      it("should remove emitter but keep stream", () => {
        const coll = "bucket";
        const options = {limit: 1};
        const options2 = {limit: 2};
        realtime["getEmitter"](coll, options);
        realtime["getEmitter"](coll, options2);
      ...
      it("should write warning if emitter does not exist", () => {
        ...
        expect(warnSpy).toHaveBeenCalledOnceWith(
          `Connection has already been closed for collection 'bucket' with options '{"limit":1}'.`
        );
      });
    });
  });
