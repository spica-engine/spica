...
describe('schema validator', () => {
  ...
  it('should assign dynamic default', async () => {
    validator.registerDefault(
      { keyword: 'dynamicdefault', type: 'string', create: () => 'dynamicvalue' });
  ...
  describe('with format', () => {
    ...
    it('should work with regex formats', () => {
      return expectAsync(validator.validate({ type: 'string', format: 'date-time' }, '1963-06-19T08:30:06.283185Z')).toBeResolved();
    })
    ...
    it('should not do any operation if format type does not match with schema', () => {
      return expectAsync(validator.validate({
        type: 'object',
        properties: {
          test: {
            type: 'number',
    ...
    describe('and coercing', () => {
      ...
      it('should coerce custom format', async () => {
        ...
        const validateSpy = jasmine.createSpy('validate function').and.callFake((data) => data == 'formatted');
        const format: Format = {
          name: 'myformat',
          type: 'string',
          coerce: coerceSpy,
