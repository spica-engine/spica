import {DependencyNamespace, InstallCommand} from "../../../src/commands/function/dependency";
...
describe("Install", () => {
  ...
  describe("run", () => {
    ...
    beforeEach(() => {
      ...
      getRequest = spyOn(installCommand.request, "getRequest");
      logError = spyOn(installCommand.logger, "error");
      postRequest = spyOn(installCommand.request, "postRequest");
    });
    ...
    function getFunctionsSuccessfully() {
      getRequest.and.returnValue(
        new Promise((resolve, reject) => {
          resolve([{_id: "id1", name: "func1"}, {_id: "id2", name: "func2"}]);
        })
      );
    }

    it("should show login error and return", async () => {
      getLoginData = spyOn(installCommand.authentication, "getLoginData").and.callFake(() => {
