...
export namespace Event {
    export class Event extends pb_2.Message {
        ...
        static deserialize(bytes: Uint8Array | pb_2.BinaryReader): Event {
            const reader = bytes instanceof Uint8Array ? new pb_2.BinaryReader(bytes) : bytes, message = new Event();
            while (reader.nextField()) {
                ...
                switch (reader.getFieldNumber()) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        reader.readMessage(message.target, (_, reader: pb_2.BinaryReader) => message.target = Target.deserialize(reader));
                        break;
    ...
    export class Target extends pb_2.Message {
        ...
        static deserialize(bytes: Uint8Array | pb_2.BinaryReader): Target {
            const reader = bytes instanceof Uint8Array ? new pb_2.BinaryReader(bytes) : bytes, message = new Target();
            while (reader.nextField()) {
                if (reader.isEndGroup())
                    break;
                switch (reader.getFieldNumber()) {
