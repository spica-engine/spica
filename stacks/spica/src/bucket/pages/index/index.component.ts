...
export class IndexComponent implements OnInit {
  ...
  async scheduleTrigger() {
    this.scheduledData = await !this.scheduledData;
    ...
    if (displayScheduleIndex > -1 && !this.scheduledData) {
      await this.displayedProperties.splice(displayScheduleIndex, 1);
    }
    if (displayScheduleIndex == -1 && this.scheduledData) {
      let lastIndex = await this.displayedProperties.lastIndexOf("$$spicainternal_actions");
      await this.displayedProperties.splice(lastIndex, 0, "$$spicainternal_schedule");
    }
    this.getData();
  }
