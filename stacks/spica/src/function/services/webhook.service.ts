...
import { DeleteWebhook, LoadWebhooks, UpdateWebhook, UpsertWebhook } from "../actions/webhook.actions";
...
export class WebhookService {
  ...
  loadWebhooks() {
    return this.http
      ...
      .pipe(tap((webhooks: WebhookResult) => this.store.dispatch(new LoadWebhooks({ webhooks: webhooks.data }))));
  }
  ...
  delete(id: string): Observable<void> {
    return this.http.delete<void>(`api:/webhook/${id}`).pipe(tap(() => this.store.dispatch(new DeleteWebhook({ id }))));
  }
