...
import {PolicyService} from "@spica-client/passport";
...
export class PolicyResourceAddComponent implements OnInit {
  ...
  getAction(action) {
    let actionIndex = this.data.statement["actions"].findIndex(
      actionInStatement => actionInStatement.action == action
    );
    return actionIndex >= 0 ? this.data.statement["actions"][actionIndex] : false;
  }

  onResourceSelection(selection: "include" | "exclude") {
    if (selection == "include") {
      this.data.action.resource = [];
    } else if (selection == "exclude") {
      this.data.action.resource = {
        //put * for each params, and build resource like '*/*'
        include: this.services[this.data.statement.module][this.data.action]
          .map(_ => "*")
          .join("/"),
        exclude: []
      };
  ...
  addInclude(resource: string, action) {
    if (resource == "") resource = this.trimResource("*/*", action.action);
    (action.resource as string[]).push(resource);
  }

  addExclude(resource: string, action) {
    if (!action.resource.exclude) action.resource = {include: "*/*", exclude: []};
  ...
  copyResources() {
    this.data.statement["actions"].map(action => {
      let currentAction = this.getAction(this.data.action);
      ...
      if (action.action != this.data.action) {
        if (this.getResourceSelection(currentAction) == "include") {
          action.resource.map((resource, resourceIndex) => {
          ...
          action.resource.exclude.map((resource, resourceIndex) => {
  ...
  private trimResource(resource, action) {
    let resourceArray = resource.split("/");
    resourceArray.map((_, resourceIndex) => {
