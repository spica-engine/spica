...
If your TypeScript projects aren't showing up with the right targets, then [view read how `@nx/js` infers projects](#how-the-typescript-plugin-infers-tasks)
...
```shell
yarn create nx-workspace my-org --template nrwl/typescript-template
```
...
```shell
pnpm create nx-workspace my-org --template nrwl/typescript-template
```
...
If you have a TypeScript project that doesn't have a build target, you can use the [`setup-build` generator](/docs/technologies/typescript/generators#setup-build) to add one:
