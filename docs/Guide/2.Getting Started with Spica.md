...
As spica does rely on **replica set** feature of MongoDB, you need to configure at least **3 members** of replica set.
...
- Setup replication between members so they can elect an primary.
  ...
  ```
- Additionally if you want keep your files on container update, you need to mount a docker volume to your **`api`** container and change `PERSISTENT_PATH` environment variable. For more info checkout: https://docs.docker.com/storage/volumes/
- Setup spica
  ```sh
