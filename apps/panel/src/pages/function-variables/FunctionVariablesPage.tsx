...
import { Button, Chip, Drawer, FlexElement, FluidContainer, Icon, Text, type TableColumn } from "oziko-ui-kit";
...
const FunctionVariablesPage = () => {
    ...
    const handleSearchChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        debounceRef.current = setTimeout(() => setSearchQuery(val), SEARCH_DEBOUNCE_MS);
    }, []);
    ...
    const envVars = useMemo(() => {
        ...
        if (!q) return allEnvVars;
        return allEnvVars.filter(ev => ev.key.toLowerCase().includes(q));
    ...
    return (
        <>
            <FlexElement dimensionX="fill" direction="vertical" gap={16} className={styles.container}>
                <FluidContainer mode="fill" dimensionX="fill" alignment="rightCenter" direction="horizontal" gap={12} root={{
                    children:
                        <SearchBar
                            loading={false}
                            inputProps={{
