...
/**
 * Enable/disable TOTP two-factor auth for a single user. Self-contained:
...
const TwoFactorSection = ({ user }: TwoFactorSectionProps) => {
  ...
  // Optimistic overrides so the UI flips immediately, before the user list
  ...
  const enabled = justEnabled || (!!user.authFactor && !justDisabled);
          ...
          confirmingDisable ? (
            <>
              <div className={styles.confirmText}>
                Disable two-factor authentication for this user? They will sign in with
                only their password.
              </div>
              {error && <div className={styles.error}>{error}</div>}
              <div className={styles.actions}>
