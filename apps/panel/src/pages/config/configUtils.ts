...
export function sanitizeForSave(
  ...
  if (result.rateLimits && typeof result.rateLimits === "object") {
    ...
    for (const [cat, rl] of Object.entries(rawRl)) {
      ...
      if (opts?.strictValidation && limitEmpty !== ttlEmpty) return null;
      cleanRl[cat] = {
...
export function updateNestedArray(
  ...
  path: string,
  updater: (list: ProviderItemWithId[]) => ProviderItemWithId[]
): Record<string, unknown> {
  const pathParts = path.split(".");
  if (pathParts.length === 1) {
    return {...prev, [path]: updater((prev[path] ?? []) as ProviderItemWithId[])};
  }
  const parent = pathParts[0];
  const child = pathParts[1];
  const parentObj = (prev[parent] ?? {}) as Record<string, unknown>;
  return {...prev, [parent]: {...parentObj, [child]: updater((parentObj[child] ?? []) as ProviderItemWithId[])}};
}
