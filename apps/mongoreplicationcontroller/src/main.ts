...
const options = yargs
  ...
  .option("bypass-quorum-check", {
    type: 'boolean',
    default: true,
    describe: "When true, the unready nodes will be added to replication by bypassing the mongodb quorum check."
  })
  .option("hostname", {
    type: 'string',
    default: os.hostname(),
...
async function initialize() {
  if ( options["from-srv"]  ) {
    ...
    if ( success ) {
      ...
      debug(`All attempts (${options["max-retry-connection"]}) to initiate the replication have failed.`);
